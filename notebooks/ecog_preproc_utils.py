...
def auto_bands(fq_min=4.0749286538265, fq_max=200.0, scale=7.0):
    """
    ...
    sds = 10 ** (np.log10(0.39) + 0.5 * (np.log10(cts)))
    return cts, sds
...
def transformData(
    ...
    if (band == "high_gamma") or (band == "broadband"):
        ...
        for i, (ct, sd) in enumerate(
            tqdm(zip(cts, sds), "applying Hilbert transform...", total=len(cts))
        ):
            hilbdat = np.zeros((raw_data.shape))
            for ch in np.arange(nchans):
                hilbdat[ch, :] = applyHilbertTransform(
                    raw_data[ch, :], raw.info["sfreq"], ct, sd
                )
            if log_transform:
        ...
        hilbmat = np.array(np.hstack((dat))).reshape(
            dat[0].shape[0], -1, dat[0].shape[1]
        )
        ...
        if do_zscore:
            ...
            hg_signal = (
                hg_signal - np.expand_dims(np.nanmean(hg_signal, axis=1), axis=1)
            ) / np.expand_dims(np.nanstd(hg_signal, axis=1), axis=1)
        ...
        if raw.annotations:  # if we rejected something reject it in HG also
            for annotation in raw.annotations:
                ...
                )  # convert start time for clinical data
